...
fn create_battery_icon(hdc: HDC, percentage: u8, is_charging: bool) -> HICON {
    unsafe {
        let size = 32; // Hi-res for modern displays
        let hdc_mem = CreateCompatibleDC(hdc);
        let hbm = CreateCompatibleBitmap(hdc, size, size);
        let hbm_mask = CreateCompatibleBitmap(hdc, size, size);
        ...
        // Main battery body (centered and larger)
        ...
        // Add charging indicator (BLACK lightning bolt)
        if is_charging && percentage < 100 {
            let brush_bolt = CreateSolidBrush(COLORREF(0x00000000)); // BLACK
            SelectObject(hdc_mem, brush_bolt);
            
            // Lightning bolt shape (larger and centered)
            let points = [
                POINT { x: 16, y: 14 },
                POINT { x: 14, y: 19 },
                POINT { x: 17, y: 19 },
...
unsafe extern "system" fn window_proc(
    ...
    match msg {
        WM_CREATE => {
            ...
            let update_interval = if DEBUG_MODE { 2000 } else { interval };
