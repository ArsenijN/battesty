    ...
    fn get_battery_status(&mut self) -> Option<(u8, String, bool)> {
        unsafe {
            ...
            if GetSystemPowerStatus(&mut status).is_ok() {
                ...
                // Save history periodically (every 10 measurements)
...
fn create_battery_icon(hdc: HDC, percentage: u8, is_charging: bool) -> HICON {
    unsafe {
        ...
        let fill_color = if is_charging {
			COLORREF(0x0000FF00) // Green when charging
		} else if percentage < 20 {
			COLORREF(0x000000FF) // Red when low
		} else if percentage < 50 {
			COLORREF(0x00FFAA00) // Orange for medium
		} else {
			COLORREF(0x00FFFFFF) // White for good levels
		};
        ...
        WM_TRAYICON => {
            if lparam.0 as u32 == WM_LBUTTONUP {
				if let Some(monitor) = &MONITOR {
					...
					// Get the LAST measurement instead of calling get_battery_status
        ...
        WM_COMMAND => {
            match wparam.0 as u32 {
                1001 => {
					...
					// After creating msg_wide and title_wide vectors, add:
					// drop(mon); // If mon variable exists
					// Then call MessageBoxW
                    MessageBoxW(hwnd, PCWSTR(msg_wide.as_ptr()), PCWSTR(title_wide.as_ptr()), MB_OK | MB_ICONINFORMATION);
                }
                1002 => {
